...
export default function Pacientes() {
    ...
    const carregarDados = async () => {
        ...
        try {
            ...
            // Map patients to tutors
            const tutorsWithPatients = tutorsList.map(tutor => ({
                ...tutor,
                pacientes: patientsList.filter(p => p.tutor_id === tutor.id)
            })).sort((a, b) => a.nome.localeCompare(b.nome));

            setTutores(tutorsWithPatients);
        } catch (error) {
    ...
    const handleSaveTutor = async (e) => {
        ...
        try {
            if (editingTutorId) {
                const tutorRef = doc(db, 'tutores', editingTutorId);
                await updateDoc(tutorRef, tutorFormData);
                toast.success('Tutor atualizado!');
            } else {
                await addDoc(collection(db, 'tutores'), tutorFormData);
    ...
    const handleDeleteTutor = async (id) => {
        if (window.confirm('Tem certeza? Isso removerá o tutor e TODOS os seus animais.')) {
            try {
                ...
                // Delete associated patients (Firestore doesn't cascade delete automatically)
                const q = query(collection(db, 'pacientes'), where('tutor_id', '==', id));
    ...
    const handleSavePatient = async (e) => {
        ...
        // Remove keys with empty strings if desired, or keep them. Firestore handles null/strings fine.
        ...
        try {
            if (editingPatientId) {
                const patientRef = doc(db, 'pacientes', editingPatientId);
